...
/**
 * A parser for Maven 2 POM, that
 * <p>
 * does not perform conversion to Ivy native format;<br/>
 * does not care for sources and javadocs;<br/>
 */
public final class BarebonePomParser implements ModuleDescriptorParser {

    private static final BarebonePomParser INSTANCE = new BarebonePomParser();

    public static BarebonePomParser getInstance() {
        return INSTANCE;
    }

    public BarebonePomParser() {
    }
    ...
    public void toIvyFile(InputStream is, Resource res, File destFile, ModuleDescriptor md)
        ...
        File origPom = new File(parent, destFile.getName() + ".original");

        if (origPom.exists()) {
            File pomName = new File(parent, destFile.getName().replace(".xml", ".pom"));

            if (!origPom.renameTo(pomName)) throw new IOException("Failed to rename POM file");

            if (destFile.exists() && !destFile.delete()) throw new IOException("Failed to delete Ivy file");
        }
    }
        ...
        try {
            ...
            if (relocation != null) {
                ...
                for (Iterator it = domReader.getDependencies().iterator(); it.hasNext();) {
                    PomDependencyData dep = (PomDependencyData) it.next();
